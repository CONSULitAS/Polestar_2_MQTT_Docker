...
client.will_set(topic=MQTT_LWT_TOPIC, payload=MQTT_LWT_MESSAGE_DEAD, qos=1, retain=True)

# setup MQTT-Client for openWB if needed
if OPENWB_PUBLISH:
    OPENWB_TOPIC = (
        "openWB/set/lp/" + str(OPENWB_LP_NUM) + "/%Soc"
    )  # TODO: use ENV string
...
def mqtt_backoff_attempt(
    client, method, max_retries=20, initial_delay=1, delay_max=300
):
    """
    ...
    start_time = (
        time.time()
    )  # Record the start time of the connection/reconnection attempts

    for attempt in range(1, max_retries + 1):
        try:
            print(
                f"    MQTT ({client_address}): attempt {attempt} to {'connect' if method == client.connect else 'reconnect'}... (waiting {delay} seconds)"
            )
            time.sleep(delay)  # Wait before trying to connect or reconnect
            method()  # Call the connect or reconnect method
            elapsed_time = time.time() - start_time  # Calculate elapsed time
            print(
                f"    MQTT ({client_address}): {'Connected' if method == client.connect else 'Reconnected'} successfully after {elapsed_time:.2f} seconds!"
            )
            ...
            delay = min(
                delay * 2, delay_max
            )  # Double the delay but do not exceed delay_max
    else:
        elapsed_time = time.time() - start_time  # Calculate elapsed time
        wait_and_die(
            f"    MQTT ({client_address}): Could not {'connect' if method == client.connect else 'reconnect'} after {max_retries} attempts. Exiting.",
...
def mqtt_on_connect(client, userdata, flags, rc, properties):
    print(
        f"    MQTT connected with result code '{rc}': {MQTT_LWT_TOPIC}={MQTT_LWT_MESSAGE_ALIVE}"
    )
...
def mqtt_connect():
    ...
    mqtt_backoff_attempt(
        client, lambda: client.connect(MQTT_BROKER, MQTT_PORT, MQTT_KEEPALIVE_INTERVAL)
    )
    client.loop_start()
...
def perform_login(email, password, path_token, cookie):
    ...
    max_age = (
        response.headers["Strict-Transport-Security"].split("max-age=")[1].split(";")[0]
    )
...
def get_token(email, password):
    ...
    path_token, cookie, code_verifier = (
        get_path_token()
    )  # code_verifier will be generated on each login
    print(" perform_login()")
    auth_code = perform_login(email, password, path_token, cookie)
    print(" get_api_token()")
    access_token, expiry_time, refresh_token = get_api_token(
        auth_code, code_verifier
    )  # code_verifier is needed to get token
    ...
    if expiry_time is None or (datetime.now() >= expiry_time - timedelta(seconds=15)):
        if refresh_token:
            ...
            try:
                ...
                access_token, expiry_time, refresh_token = refresh_access_token(
                    refresh_token
                )
            except Exception as e:
...
def get_car_data(vin, access_token):
    ...
    filtered_car_data = next(
        (car for car in car_data["getConsumerCarsV2"] if car["vin"] == vin), None
    )
...
def publish_json_as_mqtt(topic, json_obj):
    if isinstance(json_obj, dict):
        for key, value in json_obj.items():
            ...
            publish_json_as_mqtt(
                sub_topic, value
            )  # call self to resolve next json level
    else:
...
def main():
    ...
    last_car_telemetry_data = (
        None  # cache of the last battery & odometer data to detect changes
    )
    ...
    while True:
        ...
        access_token, expiry_time, refresh_token = ensure_valid_token(
            access_token, expiry_time, refresh_token, POLESTAR_EMAIL, POLESTAR_PASSWORD
        )
